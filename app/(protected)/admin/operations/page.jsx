'use client';

import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { useProfile } from '@/app/context/ProfileContext';
import { 
  ArrowLeft, 
  BookOpen, 
  Download, 
  ExternalLink, 
  ShieldCheck, 
  ShieldAlert, 
  FileText, 
  HelpCircle, 
  CreditCard, 
  Rocket, 
  AlertTriangle, 
  RotateCcw, 
  Database, 
  CheckSquare, 
  Users, 
  FileSpreadsheet,
  Loader2
} from 'lucide-react';
import { verifySuperAdminOperationsAccess } from '@/app/actions/adminOperationsActions';

export default function OperationsHandoverPage() {
  const router = useRouter();
  const { profile } = useProfile();
  const [loading, setLoading] = useState(true);
  const [authorized, setAuthorized] = useState(false);

  useEffect(() => {
    async function checkAccess() {
      try {
        const res = await verifySuperAdminOperationsAccess();
        setAuthorized(res.authorized);
      } catch (err) {
        console.error('Failed to verify access:', err);
        setAuthorized(false);
      } finally {
        setLoading(false);
      }
    }

    checkAccess();
  }, []);

  if (loading) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 flex flex-col items-center justify-center space-y-4 font-sans">
        <Loader2 size={36} className="animate-spin text-blue-900" />
        <span className="text-sm text-gray-500 font-semibold">Verifying Super Admin clearance...</span>
      </div>
    );
  }

  if (!authorized) {
    return (
      <div className="max-w-md mx-auto px-4 py-20 text-center font-sans">
        <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-md flex flex-col items-center space-y-6">
          <div className="p-4 bg-red-50 text-red-600 rounded-full">
            <ShieldAlert size={36} />
          </div>
          <div>
            <h1 className="text-xl font-bold text-gray-900">Access Restricted</h1>
            <p className="text-sm text-gray-500 mt-2">
              Operations & Handover documents are restricted exclusively to Super Administrators.
            </p>
          </div>
          <button 
            onClick={() => router.push('/admin')}
            className="w-full bg-[#002b4e] hover:bg-[#001c33] text-white text-sm font-bold py-3 rounded-xl transition-all shadow-sm cursor-pointer border-none"
          >
            Back to Platform Admin
          </button>
        </div>
      </div>
    );
  }

  const sections = [
    {
      number: '1',
      title: 'Platform Admin Quick Start Guide',
      icon: <BookOpen size={18} className="text-blue-600" />,
      description: 'Orientation, access controls, dashboard navigation, and core administrative workflows.'
    },
    {
      number: '2',
      title: 'Support Troubleshooting Runbook',
      icon: <HelpCircle size={18} className="text-teal-600" />,
      description: 'Standard diagnostics, user account troubleshooting, and issue remediation protocols.'
    },
    {
      number: '3',
      title: 'MCredits & Finance Operations Guide',
      icon: <CreditCard size={18} className="text-purple-600" />,
      description: 'Ledger monitoring, Stripe reconciliation, manual adjustments, and refund reviews.'
    },
    {
      number: '4',
      title: 'Launch-Day Cutover Runbook',
      icon: <Rocket size={18} className="text-amber-600" />,
      description: 'Step-by-step production cutover sequences, validation checkpoints, and sign-offs.'
    },
    {
      number: '5',
      title: 'Incident Escalation Matrix',
      icon: <AlertTriangle size={18} className="text-rose-600" />,
      description: 'Severity classification, operational contact chains, and management escalation paths.'
    },
    {
      number: '6',
      title: 'Deployment & Rollback Quick Reference',
      icon: <RotateCcw size={18} className="text-indigo-600" />,
      description: 'Standard deployment procedures, health verification, and rollback decision guides.'
    },
    {
      number: '7',
      title: 'Backup & Recovery Verification Checklist',
      icon: <Database size={18} className="text-emerald-600" />,
      description: 'Database snapshot verification, retention policies, and disaster recovery procedures.'
    },
    {
      number: '8',
      title: 'UAT Data Cleanup Approval Sheet',
      icon: <FileSpreadsheet size={18} className="text-cyan-600" />,
      description: 'Controlled pre-launch test data sanitization criteria and formal sign-off sheet.'
    },
    {
      number: '9',
      title: 'Launch Responsibility Assignment Sheet',
      icon: <Users size={18} className="text-violet-600" />,
      description: 'Operational role assignments, duty schedules, and management ownership matrix.'
    },
    {
      number: '10',
      title: 'Remaining Pre-Launch Actions / Approval Status',
      icon: <CheckSquare size={18} className="text-blue-700" />,
      description: 'Tracked launch readiness checklist, open dependencies, and executive approval registry.'
    }
  ];

  return (
    <div className="max-w-[1100px] mx-auto px-4 md:px-8 py-6 md:py-8 font-sans w-full pb-20">
      {/* Back Button */}
      <button
        onClick={() => router.push('/admin')}
        className="flex items-center gap-2 text-gray-500 hover:text-[#002b4e] transition-colors mb-6 text-sm font-bold cursor-pointer bg-transparent border-none p-0 outline-none"
      >
        <ArrowLeft size={16} /> Back to Admin Dashboard
      </button>

      {/* Main Header Banner */}
      <div className="bg-white border border-gray-100 rounded-3xl p-6 md:p-8 shadow-sm mb-8 relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none">
          <BookOpen size={140} />
        </div>
        <div className="relative z-10 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 border border-blue-100 text-[#0e2a4d] text-xs font-bold uppercase tracking-wider mb-4">
            <ShieldCheck size={14} className="text-blue-700" />
            Internal Management Documentation
          </div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-[#0e2a4d] leading-tight">
            MarComn Launch & Operations Handover Pack
          </h1>
          <p className="text-gray-600 text-sm md:text-base mt-2 leading-relaxed">
            Formal operational and management handover documentation for MarComn&apos;s October 2026 launch.
          </p>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-100 text-xs">
            <div>
              <span className="text-gray-400 block font-semibold">Document Version</span>
              <span className="text-gray-800 font-bold text-sm">1.0 Production</span>
            </div>
            <div>
              <span className="text-gray-400 block font-semibold">Target Release</span>
              <span className="text-gray-800 font-bold text-sm">October 2026</span>
            </div>
            <div>
              <span className="text-gray-400 block font-semibold">Access Clearance</span>
              <span className="text-emerald-700 font-bold text-sm">Super Admin Restricted</span>
            </div>
            <div>
              <span className="text-gray-400 block font-semibold">Classification</span>
              <span className="text-gray-800 font-bold text-sm">Internal Management</span>
            </div>
          </div>
        </div>
      </div>

      {/* Action Area & Quick Access */}
      <div className="bg-gradient-to-br from-[#0e2a4d] to-[#001c33] rounded-3xl p-6 md:p-8 text-white shadow-md mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2">
          <h2 className="text-lg md:text-xl font-bold flex items-center gap-2 text-white">
            <FileText size={20} className="text-blue-300" />
            Master Handover Pack (PDF)
          </h2>
          <p className="text-xs md:text-sm text-blue-100/80 max-w-xl leading-relaxed">
            Direct authenticated stream of the formal handover documentation pack. Protected behind Super Admin role validation.
          </p>
        </div>

        <div className="flex flex-wrap items-center gap-3 shrink-0 w-full md:w-auto">
          <a
            href="/api/admin/operations/handover?disposition=inline"
            target="_blank"
            rel="noopener noreferrer"
            className="flex-1 md:flex-none inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white text-xs md:text-sm font-bold px-5 py-3 rounded-xl transition-colors shadow-sm cursor-pointer no-underline"
          >
            <ExternalLink size={16} />
            View Document
          </a>
          <a
            href="/api/admin/operations/handover?disposition=attachment"
            className="flex-1 md:flex-none inline-flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-white text-xs md:text-sm font-bold px-5 py-3 rounded-xl transition-colors border border-white/20 cursor-pointer no-underline"
          >
            <Download size={16} />
            Download PDF
          </a>
        </div>
      </div>

      {/* Overview of Topics / Modules */}
      <div className="mb-8">
        <h2 className="text-base font-bold text-gray-900 mb-4 flex items-center gap-2">
          <BookOpen size={18} className="text-blue-900" />
          Included Handover Sections & Operations Runbooks
        </h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {sections.map((sec, idx) => (
            <div key={idx} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:border-blue-100 transition-all flex items-start gap-4">
              <div className="p-2.5 bg-gray-50 rounded-xl shrink-0 mt-0.5">
                {sec.icon}
              </div>
              <div className="space-y-1">
                <h3 className="font-bold text-gray-900 text-sm">
                  {sec.number}. {sec.title}
                </h3>
                <p className="text-xs text-gray-500 leading-relaxed">
                  {sec.description}
                </p>
              </div>
            </div>
          ))}
        </div>
      </div>

      {/* Security Handling Note */}
      <div className="bg-blue-50/70 border border-blue-200 rounded-2xl p-5 flex items-start gap-3">
        <ShieldCheck size={20} className="text-blue-700 shrink-0 mt-0.5" />
        <div className="text-xs text-blue-950 leading-relaxed">
          <span className="font-bold block mb-1">Super Admin Restricted Access</span>
          This documentation contains official operational procedures, incident response matrices, and launch handover protocols. Intended strictly for authorized MarComn platform management and Super Administrators.
        </div>
      </div>
    </div>
  );
}
