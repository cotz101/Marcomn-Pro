                  ...
                  {section.section_key === 'available-packages' && (
                    <div className="mt-8 pt-6 border-t border-gray-100">
                      {packages.length === 0 ? (
                        ...
                        <div>
                          <span className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">Preset Pricing Packages</span>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {packages.map((pkg) => (
                              <div
                                key={pkg.id || pkg.usdPrice}
                                className="border border-gray-150 p-4 rounded-xl flex flex-col items-center justify-center bg-slate-50/50 shadow-3xs"
                              >
                                <span className="text-base font-extrabold text-[#0e2a4d]">${pkg.usdPrice} USD</span>
                                <span className="text-xs font-semibold text-emerald-600 mt-1 flex items-center gap-1 select-none">
                                  <Coins size={12} />
                                  <span>+{pkg.mcreditAmount} MC</span>
                                </span>
                                <Link
                                  href="/profile/wallet"
                                  className="mt-4 w-full bg-[#0e2a4d] hover:bg-[#071c35] text-white text-center py-2 px-3 rounded-lg text-[10px] font-bold transition-all shadow-3xs"
                                >
                                  Purchase
                                </Link>
                              </div>
