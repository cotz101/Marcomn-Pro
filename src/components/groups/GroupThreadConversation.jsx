...
function AttachmentCard({ attachment, onOpen }) {
  ...
  if (embed) return <div className="mt-2 aspect-video max-w-full overflow-hidden rounded-lg bg-black"><iframe src={embed} title={attachment.title || `${type} preview`} className="h-full w-full" loading="lazy" allow="encrypted-media; picture-in-picture" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen /></div>;
  if (type === 'image' && imageUrl) return <button type="button" onClick={() => onOpen(attachment)} className="mt-2 block max-w-full overflow-hidden rounded-lg border border-current/20"><img src={imageUrl} alt={attachment.original_filename || 'Attached image'} className="max-h-72 w-full object-contain" /></button>;
  return <button type="button" onClick={() => onOpen(attachment)} className="mt-2 flex max-w-full items-center gap-2 overflow-hidden rounded-lg border border-current/20 bg-white/10 px-3 py-2 text-left text-xs">
