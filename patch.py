...
file_path = os.path.join(os.path.dirname(__file__), 'app/(protected)/messages/page.jsx')
...
# 3. Add App Threads and Messages fetching logic
app_fetching_logic = """
...
content = content.replace("// 3. Realtime messages subscription", app_fetching_logic)

# 4. App Realtime subscription
app_realtime_logic = """    return () => {
...
content = content.replace("    return () => {\n      supabase.removeChannel(channel);\n    };\n  }, [activeChatId]);", app_realtime_logic)

# 5. Send message logic
send_logic = """const handleSendMessage = async (e) => {
    e?.preventDefault();
...
content = content.replace("  const handleSendMessage = async (e) => {\n    e?.preventDefault();\n    if (!newMessage.trim() || !activeChatId || !currentUser.id || sending) return;", send_logic)

# 6. Fix Applications tab left panel
apps_panel_old = """{activeTab === 'applications' && (
...
apps_panel_new = """{activeTab === 'applications' && (
            <>
              {loadingApps ? (
...
content = content.replace(apps_panel_old, apps_panel_new)

# 7. Render right panel for Applications
right_panel_old = "        {activeConv && activePartner && activeTab === 'direct' ? ("
right_panel_new = """        {(activeTab === 'applications' && activeAppThread) ? (
          <>
...
cls_new_1 = """      <div 
      ...
      >"""
content = content.replace(cls_old_1, cls_new_1)
...
cls_new_2 = """      <div 
      ...
      >"""
content = content.replace(cls_old_2, cls_new_2)
