...
export async function getMCreditSetting(key, supabaseClient = null) {
  ...
  if (error || !data) {
    // Default Fallbacks
    if (key === 'mcredit_job_posting_enabled') return true;
    if (key === 'mcredit_candidate_acceptance_enabled') return true;
  ...
  if (key === 'mcredit_job_posting_enabled' || key === 'mcredit_candidate_acceptance_enabled') {
